...
int main(int argc, char** argv) {
	srand(time(NULL));
        ...
        printf("\nCurrently the Available resources : ");
        ...
        printf("\n");
        for(int i=0; i<nProcesses; i++) {
                printf("\nResource allocated to process are %d : ", i+1);
	...
	// maximum required resources
        for(int i=0; i<nProcesses; i++) {
                printf("\nMax. resource required by process are %d ", i+1);
                for(int j=0; j<nResources; j++)
	...
	safeSeq = (int *)malloc(nProcesses * sizeof(*safeSeq));
        ...
        if(!getSafeSeq()) {
                printf("\nThis is the Unsafe State! This processes leads the system to an unsafe state.\n\n");
                exit(-1);
        }
...
void* processCode(void *arg) {
	...
	// process
        ...
        printf("\tProcess Code is Completed...");
        printf("\n"); sleep(1);
